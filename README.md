...
`pdxparse` should be run from the command line. It will create a directory `output` in the current directory. Its structure is similar to that of the game directory, except that the `.txt` files are directories.

For national focuses, inside the directories is only one text file, but for events and decisions, each file in these directories is one "object": one event, one decision, etc. However, there is also a file in each folder with "_all.txt" tacked on the end which includes all of the events or decisions from that folder, using proper separators when possible (decisions use decision categories and events use ID blocks of 10).

Normally it will wait for a user input at the end so that it can be used in a command window which closes automatically (use `--nowait` to change that).
...
* HoI4
    * RHS scope parsing could be improved in a couple contexts
    * A tiny amount of lines don't have custom messages yet
